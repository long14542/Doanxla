...
namespace BasicDemo
{
    public partial class Form1 : Form
    {
        ...
        public void ReceiveThreadProcess()
        {
            ...
            MV_FG_INPUT_IMAGE_INFO_EX stDisplayInfo = new MV_FG_INPUT_IMAGE_INFO_EX();       // ch:显示的图像信息 | en:Display frame info
                    ...
                    if (CErrorCode.MV_FG_SUCCESS == nRet)
                    {
                        ...
                        if (m_nLineNum != 1)
                        {
                            ...
                            for (int i = 0; i < m_nLineNum; i++)
                            {
                                ...
                                nRet = cImgProc.DisplayOneFrame(ptrPictureBoxHandle[i], ref stDisplayInfo, RenderMode.Default);
                                if (CErrorCode.MV_FG_SUCCESS != nRet)
                                {
                                    m_cStream.ReleaseFrameBuffer(stFrameInfo);
                        ...
                        else
                        {
                            ...
                            nRet = cImgProc.DisplayOneFrame(pictureBox1.Handle, ref stDisplayInfo, RenderMode.Default);
                            if (CErrorCode.MV_FG_SUCCESS != nRet)
                            {
                                m_cStream.ReleaseFrameBuffer(stFrameInfo);
                                this.Invoke(new ShowDisplayError(DisplayError), new object[] { nRet });
