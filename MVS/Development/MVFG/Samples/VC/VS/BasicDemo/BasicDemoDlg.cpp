    ...
    while(m_bThreadState)
    {
        if (m_bStartGrabbing)
        {
            ...
            if (MV_FG_SUCCESS == nRet)
            {
                ...
                nRet = MV_FG_DisplayOneFrameEx(m_hStream, m_hwndDisplay, &stDisplayInfo, 0);
                if (MV_FG_SUCCESS != nRet)
                {
                    MV_FG_ReleaseFrameBuffer(m_hStream, &stFrameInfo);
...
int CBasicDemoDlg::SaveImage(SAVE_IAMGE_TYPE enSaveImageType)
{
    ...
    if ((Image_Bmp == enSaveImageType || Image_Jpeg == enSaveImageType) && m_stImageInfo.nImageBufLen > 0xFFFFFFFF)
    {
        LeaveCriticalSection(&m_hSaveImageMux);
    ...
    int         nRet = 0;
	uint64_t    nMaxImageLen = (uint64_t)m_stImageInfo.nWidth * m_stImageInfo.nHeight * 4 + 2048; // 确保存图空间足够，包括图像头
